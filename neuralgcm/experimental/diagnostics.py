...
class DiagnosticModule(nnx.Module):
  ...
  def __post_init__(self):
    self.initialize_diagnostics(None)

  def initialize_diagnostics(self, observations: typing.Pytree | None = None):
    """Initializes diagnostics from observations or defaults."""
    raise NotImplementedError(f'`initialize_diagnostics` on {self.__name__=}.')
...
class CumulativeDiagnostic(DiagnosticModule):
  ...
  def __call__(self, inputs):
    self.cumulative = DiagnosticValue(
        self.cumulative.value + self.extract_fn(inputs)
    )
...
class InstantDiagnostic(DiagnosticModule):
  ...
  def __call__(self, inputs):
    self.instant = DiagnosticValue(self.extract_fn(inputs))
...
class IntervalDiagnostic(DiagnosticModule):
  ...
  def next_interval(self, inputs):
    ...
    self.interval_values = DiagnosticValue(interval_values)
