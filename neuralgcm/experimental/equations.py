...
class PrimitiveEquations(ImplicitExplicitODE):
  ...
  # Note: this is generally safe to be a cached property, because we don't
  # expect self.reference_temperatures to change once the equation is
  # initialized. However if that was to happen we would end up using different
  # T_ref in different parts of the code. Other properties like orography that
  # we expect to update after initialization should not be cached.
  @functools.cached_property
  def T_ref(self) -> typing.Array:
    primitive_equation = self.equation_cls(
    ...
    return primitive_equation.T_ref
  ...
  def explicit_terms(
    ...
    return primitive_equation.explicit_terms(state)
  ...
  def implicit_terms(
    ...
    return primitive_equation.implicit_terms(state)
  ...
  def implicit_inverse(
    ...
    return primitive_equation.implicit_inverse(state, step_size)
