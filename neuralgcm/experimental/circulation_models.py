...
class DycoreWithParameterization(nnx.Module):
  ...
  def __init__(
      ...
      input_coords: coordinates.DinosaurCoordinates | None = None,
    ...
    if input_coords is None:
      ...
      self.interpolate_fn = coordinate_systems.get_spectral_interpolate_fn(
          input_coords.dinosaur_coords, self.coords.dinosaur_coords
      )
      self.reverse_interpolate_fn = (
          coordinate_systems.get_spectral_interpolate_fn(
              self.coords.dinosaur_coords, input_coords.dinosaur_coords
          )
      )
  ...
  def step_with_fixed_parameterization_tendencies(
    ...
    # prognostics, tendencies = self.coords.with_dycore_sharding(
    ...
    # prognostics = self.coords.with_dycore_sharding(prognostics)
    return prognostics
  ...
  def substep(self, prognostics: typing.Pytree):
    ...
    # next_prognostics = self.coords.with_dycore_sharding(next_prognostics)
